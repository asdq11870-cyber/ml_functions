...
def batch_eval_model(model: torch.nn.Module,
...
def training_loop(model, loss_fn, optimiser, x_train, y_train, x_test, y_test, epochs, device):
  ...
  for epoch in range(epochs+1):
    ...
    test_loss,_ = testing_loop(model, loss_fn, x_test, y_test, device)
...
def testing_loop(model, loss_fn, x_test, y_test, device):
